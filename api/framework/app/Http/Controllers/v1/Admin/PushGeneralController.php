...
use App\Http\Resources\Admin\PushGeneralResource;
...
/**
 * Class PushGeneralController
 *
 * @resource Push General
 * @package  App\Http\Controllers\v1\Admin
 */
class PushGeneralController extends Controller
{
	 ...
	 * Get all push
	 *
	 * @permission push-general.list
	 *
	 * @param Request $request
	 *
	 * @urlParam   limit integer integer static(20) Default: `20` Max: `100`
	 * @urlParam   page integer integer static(1)
	 * @urlParam   orderBy string string static(id:asc|created_at:asc) Default: `id:asc`.<br>Available fields: `id` `title` `body` `scheduled_at` `send_at` `created_at`
	 * @urlParam   search string string static(lorem) Search in the fields: `uuid` `title` `body`
	...
	public function index(Request $request)
	{
		...
		return PushGeneralResource::collection($items);
	}

	/**
	 * Create push
	 *
	 * @permission push-general.create
	 *
	 * @param Request $request
	 *
	...
	public function store(Request $request)
	{
		...
		// Fill
		$push               = new PushGeneral($request->only(['title', 'body', 'url']));
		$push->uuid         = uuid();
		...
		// Save log
		$this->log(__FUNCTION__, $push->title, 'Criou um novo push', $push);

		return (new PushGeneralResource($push))->response()->setStatusCode(201);
	}

	/**
	 * Update push
	 *
	 * @permission push-general.edit
	 *
	 * @param Request $request
	 *
	...
	public function update(Request $request)
	{
		...
		// Fill
		$push               = (new PushGeneral())->where("uuid", $request->input("uuid"))->firstOrFail();
		$push_old = clone $push;
		...
		// Save log
		$this->log(__FUNCTION__, $push->title, 'Editou um push', $push, $push_old);
	 ...
	 * Get push
	 *
	 * @permission push-general.show
	 *
	 * @param PushGeneral $push
	 *
	 * @return PushGeneralResource
	 */
	public function show(PushGeneral $push)
	{
		return new PushGeneralResource($push);
	}

	/**
	 * Remove push
	 *
	 * @permission push-general.delete
	 *
	 * @param PushGeneral $push
	 *
	 ...
	 * @throws Throwable
	 */
	public function destroy(PushGeneral $push)
	{
		if( $push->send_at )
		{
		...
		// Save log
		$this->log(__FUNCTION__, $push->title, 'Deletou um push', $push);
	 ...
	 * Export push
	 *
	 * @permission push-general.export
	 *
	 * @param Request $request
	 *
	 * @urlParam   orderBy string string static(id:asc|created_at:asc) Default: `id:asc`.<br>Available fields: `id` `title` `body` `scheduled_at` `send_at` `created_at`
	...
	public function export(Request $request)
	{
		...
		return $this->exportData($columns, $items, 'push-general', 'Exportou push geral');
	}
}
