...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Descrição",
				render: (item) => listTypeCard ? <h3>{item.descricao}</h3> : item.descricao,
			},
			{
				title    : "Criação",
			...
			},
			{
				...
				render   : (item) => <Tag color={item.send_push ? "#0acf97" : "#fa5c7c"}>{item.send_push ? "Sim" : "Não"}</Tag>
			},
			{
				title    : "Ações",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		NotificationsService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return NotificationsService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
