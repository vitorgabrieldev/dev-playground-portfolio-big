...
const config = {
	...
	permissionPrefix : "vale_rio_doce",
	list             : "Vale do Rio Doce",
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Sinopse",
				render: (item) => listTypeCard ? <h3>{item.sinopse}</h3> : item.sinopse,
			},
			{
				title : "Ordem",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		DoceRiverValleyService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return DoceRiverValleyService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
