...
const config = {
	title            : "Protetores de tela",
	permissionPrefix : "protetores_tela",
	list             : "Protetores de tela",
...
class Index extends Component {
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		ScreenProtectorsService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return ScreenProtectorsService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
