...
import { Button, Dropdown, Menu, Modal, Spin, Tag, Avatar } from "antd";
...
const config = {
	title            : "Conteúdos ver mais",
	permissionPrefix : "ver_mais",
	list             : "Conteúdos ver mais",
...
class Index extends Component {
				...
				render   : (item) => {
					return (
						<div style={listTypeCard ? {paddingTop: 30, paddingBottom: 15} : {}}>
							<a onClick={() => this.onImagePreview(item.icone)}>
								<Avatar size={this.state.desktopDown || listTypeCard ? 100 : 60} src={this.state.desktopDown ? item.icone : item.icone} />
							</a>
						</div>
					)
				}
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		contentsSeeMoreService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return contentsSeeMoreService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
