...
const config = {
	title            : "Categorias | Treinamentos",
	permissionPrefix : "categorias_treinamento",
	list             : "categorias_treinamento",
...
class Index extends Component {
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		categoriesTrainingService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return categoriesTrainingService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
