...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Categoria",
				render: (item) => listTypeCard ? <h3>{item?.categoria?.name}</h3> : item?.categoria?.name,
			},
			{
				title : "Título",
			...
			},
			{
				...
				render: (item) => {
				  return listTypeCard
					? <h3 dangerouslySetInnerHTML={{ __html: item.descricao }} />
					: <span dangerouslySetInnerHTML={{ __html: item.descricao }} />;
				}
			},						
			{
				title : "Duração",
				render: (item) => listTypeCard ? <h3>{item.duracao}</h3> : item.duracao,
			},
			{
				title : "Ordem",
			...
			},
			{
				...
				className: "active no-ellipsis",
				render   : (item) => this.state.activeLoadings.indexOf(item.uuid) !== -1 ? <Spin indicator={<i className="fad fa-spinner-third fa-spin" />} /> :
					<Tag color={item.destaque ? "#0acf97" : "#fa5c7c"}>{item.destaque ? "Sim" : "Não"}</Tag>
			},
			{
				title    : "Ações",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		{
			data.categoria_id = filters.categoria_id;
		}
		...
		TrainingsService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return TrainingsService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
