...
export const getAll = (options) => {
	...
	if( options.hasOwnProperty("perfil_id") )
	{
		params.push(`perfil_acesso_id=${options.perfil_id}`);
	}

	if (options.hasOwnProperty("preferencias")) {
		options.preferencias.forEach(uuid => {
		  params.push(`preferencias[]=${uuid}`);
		});
	}
